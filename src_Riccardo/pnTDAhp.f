...
!       double precision, allocatable, save :: sixj1(:,:,:,:,:,:)
       ...
       open(1,file='pnTDAhp_en.out',status='unknown',form='formatted')
        write(1,*)
       open(2,file='pnTDAhp_dim.out',status='unknown',form='formatted')
        write(2,*)
       open(3,file='pnphonhp_str.out',status='unknown',form='formatted')
        write(3,*)
       open(4,file='pnTDAhp_store.out',status='unknown',
     &                                               form='unformatted')
...
!       call dimm(id,nosc_TDA)
      ...
      do ipar=-1,1,2
        ...
        write(*,*) 'pnTDAhp calculation for pi=',ipar,'J=',Jp
...
!        if(i1.gt.0) allocate(list_phon(i1))
          ...
          do j=1,i1
           ...
           if(tz1.eq.-1.and.tz2.eq.-1) then
            ...
            if(i.eq.j) TDA_matrix(i,j)=-lhfp(h1)%ei+lhfn(p1)%ei
             do Jpp=0,jmax
             TDA_matrix(i,j)=TDA_matrix(i,j)+Vpn(h1,p2,h2,p1,Jpp)
     &        *phase*dble(2*Jpp+1)*sixj1(lhfn(p2)%j2,lhfp(h1)%j2,Jpp,
     &                                    lhfn(p1)%j2,lhfp(h2)%j2,Jp)
             enddo
           endif
          enddo
         enddo
        ...
        if(if_QTDA.eq.1) then
         write(*,*) 'Stop: pnTDAhp not coded in the qp version.'
         stop
        endif
     ...
     &                 lhfn(m2)%j2,lhfn(m2)%ei,TDA_matrix(j,i)**2.d0
      ...
      enddo
       ...
       write(*,*) 'pnTDAhp calculation has finished.'

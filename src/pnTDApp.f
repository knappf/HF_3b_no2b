...
!       double precision, allocatable, save :: sixj1(:,:,:,:,:,:)
       ...
       open(1,file='pnTDApp_en.out',status='unknown',form='formatted')
        write(1,*)
       open(2,file='pnTDApp_dim.out',status='unknown',form='formatted')
        write(2,*)
       open(3,file='pnphonpp_str.out',status='unknown',form='formatted')
        write(3,*)
       open(4,file='pnTDApp_store.out',status='unknown',
     &                                               form='unformatted')
...
!       call dimm(id,nosc_TDA)
      ...
      do ipar=-1,1,2
        ...
        write(*,*) 'pnTDApp calculation for pi=',ipar,'J=',Jp
...
!        if(i1.gt.0) allocate(list_phon(i1))
         ...
         do i=1,i1
          ...
          do j=1,i1
           ...
           if(tz1.eq.-1.and.tz2.eq.-1) then
            phase=dble(1)
            if(i.eq.j) TDA_matrix(i,j)=lhfn(p1)%ei+lhfp(h1)%ei
            TDA_matrix(i,j)=TDA_matrix(i,j)+Vpn(h1,p1,h2,p2,Jp)
     &        *phase
           endif
          enddo
         enddo
        ...
        if(if_QTDA.eq.1) then
         write(*,*) 'Stop: pnTDApp not coded in the qp version.'
         stop
        endif
     ...
     &                 lhfn(m2)%j2,lhfn(m2)%ei,TDA_matrix(j,i)**2.d0
      ...
      enddo
       ...
       write(*,*) 'pnTDApp calculation has finished.'

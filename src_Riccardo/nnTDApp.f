...
!       enddo
       ...
       open(1,file='nnTDApp_en.out',status='unknown',form='formatted')
        write(1,*)
       open(2,file='nnTDApp_dim.out',status='unknown',form='formatted')
        write(2,*)
       open(3,file='nnhonpp_str.out',status='unknown',form='formatted')
        write(3,*)
       open(4,file='nnTDApp_store.out',status='unknown',
     &                                               form='unformatted')
...
!       enddo
      ...
      do ipar=-1,1,2
        ...
        write(*,*) 'nnTDApp calculation for pi=',ipar,'J=',Jp

       if(if_QTDA.eq.0) then
        i1=0
        do i=min_n,max_n !1,id
         if(dabs(lhfn(i)%ui**2.d0-1.d0).lt.1.d-2) then
          do j=i,max_n !min_n,max_n !1,id
...
!        if(i1.gt.0) allocate(list_phon(i1))
       ...
       if(i1.gt.0) then
        ...
        if(if_QTDA.eq.0) then
         do i=1,i1
          ...
          h1=ph(i)%q1  !neutron index
          tz1=ph(i)%tz
          do j=1,i1
           p2=ph(j)%q2  !neutron index
           h2=ph(j)%q1  !neutron index
           tz2=ph(j)%tz
           if(tz1.eq.+1.and.tz2.eq.+1) then
            phase=dble(1)
            if(i.eq.j) TDA_matrix(i,j)=lhfn(p1)%ei+lhfn(h1)%ei
            TDA_matrix(i,j)=TDA_matrix(i,j)+Vnn_me(h1,p1,h2,p2,Jp)
     &        *phase
           endif
          enddo
         enddo
     ...
     &             'nn',lhfn(m1)%l,lhfn(m1)%j2,lhfn(m1)%ei,lhfn(m2)%l,
...
!       deallocate(sixj1)
       ...
       write(*,*) 'nnTDApp calculation has finished.'

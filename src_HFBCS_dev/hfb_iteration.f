...
!********************************************************************************
       ...
       write(*,*) 'The BCS calculation starts.'
       write(*,*)
       write(*,*) 'E_HFB,       lambda_p,         lambda_n'

       if(AZBCS.gt.0) ifp_hfb=.true.
       if(ANBCS.gt.0) ifn_hfb=.true.
...
!        lhfn(i)%vi=lhfn(i)%vi*(-1)**((lhfn(i)%j2-1)/2)
     ...
     &                *R_val(levn(k)%nn,levn(k)%l,levn(k)%j2,bos1,radi)
       ...
       open(47,file='rad_n_dens.out',status='unknown',form='formatted')
       rad_sum=0.d0
       dx=sizebox/dble(igrid2)
       do i=0,igrid2
        radi=dble(i)*sizebox/dble(igrid2)
       ...
       open(47,file='rad_p_dens.out',status='unknown',form='formatted')
       rad_sum=0.d0
       dx=sizebox/dble(igrid2)
       do i=0,igrid2
        radi=dble(i)*sizebox/dble(igrid2)
       ...
       open(47,file='rad_t_dens.out',status='unknown',form='formatted')
       rad_sum=0.d0
       dx=sizebox/dble(igrid2)
       do i=0,igrid2
        radi=dble(i)*sizebox/dble(igrid2)

     ...
     &                .and.levp(k)%j2.eq.levp(l)%j2) then
            do Jp=0,jmax
            val=val+Vpp_me(i,k,j,l,Jp)
     &              *rhop_HFB(l,k)*dble(2*Jp+1)/dble(levp(i)%j2+1)
     &             +Vpn_me(i,k,j,l,Jp)
     &              *rhon_HFB(l,k)*dble(2*Jp+1)/dble(levp(i)%j2+1)
            enddo
           endif
          enddo
         enddo
         h1(i,j)=kin_p(i,j)+val
        enddo
       enddo
...
!               val2=val2
!     &          +0.5d0*rhop_HFB(lp1(m),lp1(k))*rhop_HFB(lp1(n),lp1(l))
!     &            *V3BNO2(k,l,m,n,Jp,itpoi1(1,1,3),klpoi1(i,j))
!     &            /dble(lev1pn(i)%j2+1)
!     &     +(1.d0/3.d0)*rhon_HFB(lp1(m),lp1(k))*rhon_HFB(lp1(n),lp1(l))
!     &            *V3BNO2(k,l,m,n,Jp,itpoi1(1,1,1),klpoi1(i,j))
!     &            /dble(lev1pn(i)%j2+1)
!     &     +(1.d0/6.d0)*rhon_HFB(lp1(m),lp1(k))*rhon_HFB(lp1(n),lp1(l))
!     &            *V3BNO2(k,l,m,n,Jp,itpoi1(1,1,3),klpoi1(i,j))
!     &            /dble(lev1pn(i)%j2+1)
